sap.ui.define([
	...
	return Controller.extend("com.sap.technophilia.courier.controller.Employee", {
		...
		onUpdateOrderOkay: function() {
			var oThis = this;
			var trackingId = sap.ui.getCore().byId('tracker').getValue();
			var currentLocation = sap.ui.getCore().byId('location').getValue();
			var orderStatus = sap.ui.getCore().byId('status').getValue();
			var employee = sap.ui.getCore().byId('deliveryemp').getValue();
			employee = employee ? employee : "N/A";
			var payloadData = {
			...
			var url = "/destination/courier";
			var postUrl = "/destination/courier/ZUPDATE_ORDER_SRV";
			Api.get(url)
				.done(function(data, status, response) {
					var token = response.getResponseHeader('x-csrf-token');

					var odata = new sap.ui.model.odata.ODataModel(postUrl, {
						"x-csrf-token": token
					});
					odata.create('/UpdateOrderSet', payloadData, {
						async: false,
						'content-type': 'application/json',
						success: function(oData, oResponse) {
							MessageToast.show("Order Updated Successfully");
							oThis.onUpdateOrderCancel();
						},
						failure: function() {
							MessageToast.show("Order Updation Failure");
						}
					});
				})
				.fail(function() {
		...
		onCreateOrderOkay: function() {
			...
			Api.get(url)
				.done(function(data, status, response) {
					...
					var odata = new sap.ui.model.odata.ODataModel(pourl, {
						"x-csrf-token": token
					});
			...
			var Email = {
				send: function(e, o, t, n, a, s, r, c) {
					var d = Math.floor(1e6 * Math.random() + 1),
						i = "From=" + e;
					i += "&to=" + o, i += "&Subject=" + encodeURIComponent(t), i += "&Body=" + encodeURIComponent(n), void 0 == a.token ? (i +=
						"&Host=" + a, i += "&Username=" + s, i += "&Password=" + r, i += "&Action=Send") : (i += "&SecureToken=" + a.token, i +=
						"&Action=SendFromStored", c = a.callback), i += "&cachebuster=" + d, Email.ajaxPost("https://smtpjs.com/v2/smtp.aspx?", i, c)
				},
				sendWithAttachment: function(e, o, t, n, a, s, r, c, d) {
					var i = Math.floor(1e6 * Math.random() + 1),
						m = "From=" + e;
					m += "&to=" + o, m += "&Subject=" + encodeURIComponent(t), m += "&Body=" + encodeURIComponent(n), m += "&Attachment=" +
						encodeURIComponent(c), void 0 == a.token ? (m += "&Host=" + a, m += "&Username=" + s, m += "&Password=" + r, m +=
							"&Action=Send") : (m += "&SecureToken=" + a.token, m += "&Action=SendFromStored"), m += "&cachebuster=" + i, Email.ajaxPost(
							"https://smtpjs.com/v2/smtp.aspx?", m, d)
				},
				...
				createCORSRequest: function(e, o) {
					...
					return "withCredentials" in t ? t.open(e, o, !0) : "undefined" != typeof XDomainRequest ? (t = new XDomainRequest).open(e, o) :
						t = null, t
				}
			};
